# ALPACART — Estado del proyecto nuevo

> Última actualización: 2026-08-25 (institucional migrada: la migración está completa)
> Stack: FastAPI + SQLAlchemy 2.0 + PostgreSQL 18 · React 19 + TypeScript + Vite

El proyecto viejo (`../backend`, `../frontend`) queda intacto como referencia. Todo
lo nuevo vive acá.
...
| Backend FastAPI | ✅ 94 endpoints, 104/104 en smoke test | 8010 |
...
docker compose -f alpacart_new/docker-compose.yml up -d

# 2. Backend  (desde alpacart_new/backend)
.venv/Scripts/python.exe -m uvicorn app.main:app --port 8010 --reload

# 3. Tienda  (desde alpacart_new/frontend/tienda)
npm run dev

# 4. Dashboard  (desde alpacart_new/frontend/dashboard)
npm run dev

# 5. Institucional  (desde alpacart_new/frontend/institucional)
npm run dev
```
...
.venv/Scripts/python.exe smoke_test.py              # 104 verificaciones

cd alpacart_new/frontend/tienda
npx tsc --noEmit && npm run build

cd alpacart_new/frontend/dashboard
npx tsc -b && npm run build

cd alpacart_new/frontend/institucional
npx tsc -b && npm run build
```
...
La app vieja (`../frontend/pagina-institucional`, React JS) queda como
referencia igual que el resto del stack anterior; ya no hace falta levantarla.
